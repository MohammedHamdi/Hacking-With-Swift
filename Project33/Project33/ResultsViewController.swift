...
//  Project33
//
//  Created by Mohammed Hamdi on 10/5/19.
...
class ResultsViewController: UITableViewController {
        ...
        CKContainer.default().publicCloudDatabase.save(whistleRecord) { [unowned self] record, error in
            DispatchQueue.main.async {
                if error == nil {
                    ...
                    let ac = UIAlertController(title: "Error", message: "There was a problem submitting youe suggestion: \(error!.localizedDescription)", preferredStyle: .alert)
    ...
    @objc func downloadTapped() {
        ...
        CKContainer.default().publicCloudDatabase.fetch(withRecordID: whistle.recordID) { [unowned self] record, error in
            if let error = error {
                DispatchQueue.main.async {
                    let ac = UIAlertController(title: "Error", message: "There was an error: \(error.localizedDescription), Please try again.", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
                    self.present(ac, animated: true)
